...
/*     debug(true);
    ...
    this.registerCapabilityListener('locked', async (open) =>{
      ...
      this.log('Garage Door Locked state set to', !open)
    } catch (e) {
      this.log("Failed to set locked", e);
    } 
    });
    this.registerCapabilityListener('garagedoor_closed', async (open) => {
      try{
      await this.writeBool(V1_GARAGE_DOOR_CONTROLLER_DATA_POINTS.door_opening, !open)
      this.log('Garage Door Ordered Opened set', !open)
      } catch (e) {
        this.log("Failed to set Garage Door Opened", e);
      }
    });
...
async processDatapoint(data) {
 ...
 // Prvent toggling of the capbiltiy while moving, i.e. when closing as DP3 is sent every 30s it may toggle otherwise
          if (!this.door_moving) {
          try {
            await this.setCapabilityValue('garagedoor_closed', !parsedValue);
            await this.setCapabilityValue('locked', !parsedValue);
          } catch (e) {
              this.log("Failed to set garagedoor_closed", e);
          }
        }
          break;
